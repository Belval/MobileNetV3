...
def predict_images(paths, model, output_directory):
    ...
    for i in range(predictions.shape[0]):
        for j in range(predictions.shape[-1]):
            new_w, new_h = (int(orig_sizes[i][0] * 512 / orig_sizes[i][1]), 512) if orig_sizes[i][0] > orig_sizes[i][1] else (512, int(orig_sizes[i][1] * 512 / orig_sizes[i][0]))
            img = Image.new('RGB', (new_w, new_h))
            ret, th = cv2.threshold(predictions[i, 0:images[0].shape[1], 0:images[0].shape[0], j], 0, 255, cv2.THRESH_OTSU)
            img.paste(Image.fromarray(th).resize((512, 512)), (int(img.size[0] / 2 - 256), int(img.size[1] / 2 - 256)))
...
def evaluate():
    ...
    # Load model
    model = MobileNetV3LiteRASPP(shape=(512, 512, 3), n_class=args.class_count, task="segmentation")
