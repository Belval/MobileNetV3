...
def coco_data_generator(path, batch_size, class_count):
    ...
    def generator(coco, path, batch_size, class_count):
        ...
        while True:
            ...
            labels = np.zeros(
                (batch_size, 1024 // 8, 1024 // 8, class_count), dtype=np.uint8
            )
            count = 0
        ...
        while True:
            ...
            for f in files:
                ...
                images[count, :, :, :] = resize_and_crop(
                    arr, picture_size, centering=centering
                )
                for l, mask in masks:
                    labels[count, :, :, l] += resize_and_crop(
    ...
    def generator(images_dir, masks_dir, batch_size, class_count):
        while True:
            ...
            for image_filename in os.listdir(images_dir):
                ...
                mask = Image.open(
                    os.path.join(masks_dir, f"{image_filename[:-4]}_segmentation.png")
                )

                if model_size == "large":
                    labels[count, :, :, 0] = resize_and_crop(
                        mask, picture_size // 8, centering=centering, rgb=False
                    )
                else:
                    labels[count, :, :, 0] = resize_and_crop(
...
def isic_classification_data_generator(
    images_dir, labels_file, batch_size, class_count, picture_size
):
    label_dict = {}
    ...
    def generator(images_dir, labels_file, batch_size, class_count, picture_size):
        while True:
            images = np.zeros(
                (batch_size, picture_size, picture_size, 3), dtype=np.uint8
            )
    ...
    return (
        generator(images_dir, labels_file, batch_size, class_count, picture_size),
        len(os.listdir(images_dir)),
        weights,
    )


def isic_classification_augmented_data_generator(
    images_dir, labels_file, batch_size, class_count, picture_size
):
...
def isic_mixed_data_generator(
    ...
    def segmentation_generator():
        while True:
            for image_filename in os.listdir(seg_images_dir):
                ...
                mask = Image.open(
                    os.path.join(
                        seg_mask_dir, f"{image_filename[:-4]}_segmentation.png"
                    )
                )
    ...
    def generator():
        while True:
            ...
            yield images, {"segme_out": seg_labels, "class_out": class_labels}

    return (
        generator(),
        len(os.listdir(class_images_dir)) + len(os.listdir(seg_images_dir)),
        weights,
    )
...
if __name__ == "__main__":
    train_generator, c1 = isic_data_generator(
        "../data/isic/imgs", "../data/isic/masks", batch_size=32, class_count=1
    )
