...
def coco_data_generator(path, batch_size, class_count):
    ...
    def generator(coco, path, batch_size, class_count):
        ...
        files = { int(f.split(".")[0]):os.path.join(images_directory, f) for f in os.listdir(images_directory) }
        ...
        for image_id, segmentation in coco.imgToAnns.items():
            images[count, :, :, :] = np.array(Image.open(files[image_id]).resize((448, 448)).convert("RGB"))
            for ann in segmentation:
                labels[count, :, :, ann['category_id'] - 1] += np.array(Image.fromarray(coco.annToMask(ann)).resize((448 // 8, 448 // 8)))
