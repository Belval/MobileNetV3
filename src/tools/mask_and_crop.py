...
def main():
    ...
    for f in os.listdir(images_dir):
        ...
        mask_p = os.path.join(
            masks_dir, ".".join(f.split(".")[:-1]) + "_segmentation.png"
        )
        try:
            top, bottom, left, right = get_bbox(
                np.array(Image.open(mask_p).convert("1"))
            )
            cropped_img = Image.fromarray(
                np.array(Image.open(img_p))[top:bottom, left:right, :]
            )
